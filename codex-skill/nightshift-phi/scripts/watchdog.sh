...
while true; do
    ...
    # 构建重启命令（通过读取 prompt 文件传参，避免 "$nightshift-phi" 被 shell 二次展开）
    if [ -f "$PROMPT_FILE" ]; then
        RUN_CMD="cd $WORK_DIR && codex exec --full-auto -s workspace-write \"\$(cat $STATE_DIR/launch_prompt.txt)\" 2>&1 | tee -a $STATE_DIR/session.log"
    else
        FALLBACK_PROMPT_FILE="$WORK_DIR/$STATE_DIR/watchdog_prompt.txt"
...
不要停，直到我手动停止。
EOF
        RUN_CMD="cd $WORK_DIR && codex exec --full-auto -s workspace-write \"\$(cat $STATE_DIR/watchdog_prompt.txt)\" 2>&1 | tee -a $STATE_DIR/session.log"
    fi
