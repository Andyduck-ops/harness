#!/bin/bash
# watchdog.sh — 看门狗：codex 进程退出后自动重启（语义等价重放）
...
在 $WORK_DIR 工作。只在 references/lanes/philosophy/ 范围内读写（禁止写其他 lane）。
读取 references/lanes/philosophy/_master_index.md 识别空白区域并持续探索。
